...
export default function AmbrolauriPage() {
  const { pages } = useContext(I18nContext)
  const { lang } = useLang()
  const navigate = useNavigate()
  const contentRef = useRef(null)
  // Fall back to English content until per-language translations are added.
  const page = pages.ambrolauri || enPages.ambrolauri
  const seo = getSEO('ambrolauri', lang)
  ...
  const jsonLd = useMemo(() => {
    const url = `${SITE_URL}/${lang}/destinations/ambrolauri`
    return {
      ...
      '@graph': [
        ...
        },
        {
          ...
          itemListElement: [
            ...
            { '@type': 'ListItem', position: 2, name: 'Ambrolauri', item: url },
          ],
        },
        {
          '@type': 'FAQPage',
  ...
  useSEO({ ...seo, lang, path: 'destinations/ambrolauri', image: HERO_IMAGE, jsonLd })
